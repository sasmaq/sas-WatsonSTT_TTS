...
def watson_assistant(text):
    ...
    WAapiKey = 'dAVIoFt4z2yfWe2zjamBEJimDoAoxoYE_9FQmwwxUh5i'
    WAurl = 'https://api.eu-de.assistant.watson.cloud.ibm.com/instances/ed320605-9ccf-4e95-b918-b80698dbaedc'
    assistant_id = '2f68dd98-0bf9-4f80-978b-0def32d79cd1'

    WAauthenticator = IAMAuthenticator(WAapiKey)
    assistant = AssistantV2(
        version='2018-09-20',
        authenticator=WAauthenticator
    )

    assistant.set_service_url(WAurl)

    ##Create A Session

    session = assistant.create_session(assistant_id).get_result()
    sesssion_json = json.dumps(session, indent=2)
    session_dict = json.loads(sesssion_json)
    ...
    finalResponse = response_dict["output"]["generic"][0]["text"]
    print(finalResponse)
    text_to_speech(finalResponse)

    # Delete a session
    assistant.delete_session(assistant_id, session_id)

def text_to_speech(text):
    TTSapiKey = 'vUQA9dtUfDg-VichBWJLTUaWfkp8edKY5xKBID6EwGvN'
    TTSurl = 'https://api.us-south.text-to-speech.watson.cloud.ibm.com/instances/fad0fc7d-8f68-4999-9e71-81deda708ae0'
